...
import { Loader2, Search, ChevronRight, Sparkles, Pencil, Trash2, Wand2, Target } from 'lucide-react';
...
function renderBody(body: string) {
  ...
  const flushTable = () => {
    ...
    blocks.push(
      <div key={`tbl-${blocks.length}`} className="my-3 overflow-x-auto -mx-1">
        <table className="w-full text-xs border border-border/50 rounded-md overflow-hidden">
          <thead className="bg-muted/60">
            <tr>
              {header.map((h, i) => (
                <th key={i} className="px-2 py-1.5 text-left font-semibold border-b border-border/50">
                  {renderInline(h)}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {bodyRows.map((row, ri) => (
              <tr key={ri} className="odd:bg-background even:bg-muted/20">
                {row.map((c, ci) => (
                  <td key={ci} className="px-2 py-1.5 align-top border-t border-border/40 leading-relaxed">
                    {renderInline(c)}
                  </td>
                ))}
              </tr>
...
export default function MySop() {
  ...
  const refresh = async () => {
    ...
    const [{ data: c }, { data: e }] = await Promise.all([
      supabase.from('shop_kb_categories' as any).select('*').eq('type', 'sop').order('sort_order'),
      supabase.from('shop_kb_entries' as any).select('*').eq('type', 'sop').order('sort_order'),
    ]);
    setCats((c as any) || []);
    setEntries((e as any) || []);
    setLoading(false);
  ...
  const okrHighlights = useMemo(
    () => entries.filter(e => (e.tags || []).includes('okr-highlight')),
    [entries],
  );
    ...
    try {
      ...
      if (!cat && d.category_name?.trim()) {
        const { data: newCat, error: e2 } = await supabase
          ...
          .insert({ type: 'sop', name: d.category_name.trim(), sort_order: cats.length })
          .select().single();
        if (e2) throw e2;
        cat = newCat as any;
        await refresh();
  ...
  const saveDraft = async () => {
    ...
    const { error } = id
      ? await supabase.from('shop_kb_entries' as any).update({ ...payload, type: 'sop' }).eq('id', id)
      : await supabase.from('shop_kb_entries' as any).insert({ ...payload, type: 'sop' });
    setSaving(false);
  ...
  return (
    <>
      <PageHeader title="门店手册" back="/me" />
      <div className="container mx-auto max-w-screen-md px-3 py-3 space-y-3">

        {/* Hero */}
        <Card className="relative overflow-hidden border-primary/30 bg-gradient-to-br from-primary/15 via-primary/5 to-background p-4">
          <div className="flex items-start gap-2">
            <div className="flex-1">
              <h1 className="text-base font-bold tracking-wide">BOOMER·OFF Vintage 门店手册</h1>
              <p className="text-xs text-primary/90 font-medium mt-1">虽古但新 · 信任可见</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">国内首家标准化中古连锁品牌</p>
            </div>
            <Target className="w-6 h-6 text-primary/60 shrink-0" />
          </div>
          <p className="text-[11px] text-muted-foreground mt-3 leading-relaxed">
            这不是一份机械的 SOP,而是我们的 <strong className="text-foreground font-semibold">OKR 自驱手册</strong> 和品牌运营核心思想。
          </p>
        </Card>

        {/* OKR 概览卡 */}
        {okrHighlights.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {okrHighlights.map(e => (
              <button
                key={e.id}
                onClick={() => setOpenEntry(e)}
                className="text-left p-2.5 rounded-lg border border-border/60 bg-card hover:border-primary/50 hover:bg-primary/5 transition-colors"
              >
                <p className="text-xs font-bold leading-snug line-clamp-2">{e.title}</p>
                ...
                <div className="flex items-start gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-1.5">
                      ...
                      {(e.tags || []).filter(t => t !== 'okr-highlight').slice(0, 3).map((t) => (
                        <span
                          key={t}
        ...
        <SheetContent side="bottom" className="max-h-[88vh] overflow-y-auto">
          {openEntry && (
            <>
              ...
              {openEntry.tags?.filter(t => t !== 'okr-highlight').length > 0 && (
                <div className="mt-4 pt-3 border-t border-border/50 flex flex-wrap gap-1.5">
                  {openEntry.tags.filter(t => t !== 'okr-highlight').map((t) => (
                    <span
                      key={t}
      ...
      <Dialog open={!!draft} onOpenChange={(o) => !o && setDraft(null)}>
        <DialogContent className="max-w-lg max-h-[90vh] flex flex-col p-0 gap-0">
          <DialogHeader className="px-6 pt-6 pb-2 shrink-0">
            <DialogTitle>{draft?.id ? '编辑手册条目' : '新增手册条目'}</DialogTitle>
          </DialogHeader>
          {draft && (
