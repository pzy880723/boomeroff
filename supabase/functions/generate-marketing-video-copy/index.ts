...
${OWN_BRAND_LOCK_ZH}
${kbBlock}
你的任务:为一条已经拍好的 ${duration} 秒短视频写一条**小红书发帖文案**。
要求:
- 文案必须紧扣视频里真的说了什么、拍了什么(下面会给你视频脚本摘要),不要发挥无关内容。
- 单条输出(不要多个候选),风格自然、有质感,不用堆砌 emoji 也不用喊话式感叹号轰炸。
- 标题 ≤22 字,能一眼看出这条视频是什么,可带 1 个 emoji。
- 正文 120–200 字,3–5 短段,用户读起来像随手记;可以引用视频里的一两句台词。
- hashtags 6–10 个,先品类/风格,再情绪/人群。
- 首评 1 句,引导互动或补充信息。
