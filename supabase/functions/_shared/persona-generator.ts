...
// 关键设计:
//   1) 调 AI 前先本地随机抽 { ageBucket, groupType },把年龄段/组合形式硬塞进 prompt
//      以保证真正随机 —— 老人、中年、年轻、情侣、一家三口 都会出现。
//   2) 品类只影响气质和组合，不再决定年龄；中古/老物件不等于只能由老人出镜。
//   3) 外观和面部质感都用"反 AI 感"硬约束,追求真人 vlog 感,禁止塑料脸/奇装异服。
...
const CATEGORY_TABLE: CategoryWeights[] = [
  { keywords: ['瓷器', '古董', '中古', '文玩', '字画', '旗袍', '茶器', '茶具', '老物件', '古玩', '收藏'] },
  { keywords: ['玩具', '潮玩', '盲盒', '谷子', '动漫', '二次元', '手办', '模型'] },
  { keywords: ['母婴', '亲子', '绘本', '童装', '婴儿', '儿童'], familyBoost: 30 },
  { keywords: ['美妆', '首饰', '包包', '穿搭', '潮牌', '球鞋', '彩妆', '香水'] },
  { keywords: ['家居', '咖啡器具', '原木', '北欧', '家具', '灯具'] },
  { keywords: ['户外', '运动', '装备', '工具', '露营', '钓鱼'] },
  { keywords: ['食品', '餐饮', '烘焙', '小吃', '甜品', '零食'] },
];

const DEFAULT_AGE_WEIGHTS: [number, number, number] = [40, 40, 20];

// 各年龄段真实会讲 / 绝对不会讲的场景话题,喂给脚本 AI 和 persona AI,防止老头子讲"暑假来逛"
const AGE_TOPIC_HINTS: Record<AgeBucket, { ok: string; ban: string }> = {
  young: {
    ok: '周末逛街/朋友安利/入坑/打卡/攒钱买/下班放松/约会/闺蜜安利；只有明确活动要求时才使用节日或假期话题',
...
function weightedPick<T>(items: { item: T; w: number }[], random: () => number = Math.random): T {
...
function pickPersonaSlot(assetTags: string[], assetCategories: string[], random: () => number = Math.random): {
  ageBucket: AgeBucket;
  age: number;
  groupType: GroupType;
} {
  ...
  const ageBucket = weightedPick<AgeBucket>([
    { item: 'young', w: DEFAULT_AGE_WEIGHTS[0] },
    { item: 'middle', w: DEFAULT_AGE_WEIGHTS[1] },
    { item: 'senior', w: DEFAULT_AGE_WEIGHTS[2] },
  ], random);
  ...
  const age = lo + Math.floor(random() * (hi - lo + 1));
...
export function pickPersonaSlotForTest(
  assetTags: string[],
  assetCategories: string[],
  randomValue: number,
): { ageBucket: AgeBucket; age: number; groupType: GroupType } {
  const fixed = Math.min(0.999999, Math.max(0, randomValue));
  return pickPersonaSlot(assetTags, assetCategories, () => fixed);
}
...
品类只用来指引气质/兴趣角度/口头禅/穿着风格,不能推翻上面随机抽到的年龄段。
...
- young 讲周末逛街、朋友安利、打卡;middle 讲下班顺路、带娃、送礼、孝敬爸妈。除非任务明确给了活动,任何年龄都不要主动硬蹭暑假或节日。
...
【口播话题 · 必须符合角色年龄(${p.age} 岁 ${ageBucketZhShort(inferBucket(p))})】
- 推荐话题:${AGE_TOPIC_HINTS[inferBucket(p)].ok}
- 严禁话题:${AGE_TOPIC_HINTS[inferBucket(p)].ban}
- 例:senior(老年)不许说"暑假来逛""开学季""追星入坑",要说"退休了多出来走走""接孙子路上顺道进来""老伙计推荐的""年轻时候就喜欢"。
- couple/family 里若有老人,老人开口只讲老人话题;暑假/孩子/追星话题只让年轻或中年成员讲。
