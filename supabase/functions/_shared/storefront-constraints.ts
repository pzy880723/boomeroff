...
export function usesOpenFrontMallConstraint(context: unknown): boolean {
  ...
  return /(商场\s*B1|B1\s*层|地下一层)/i.test(text) && /(开放式|无门|8\s*米|八\s*米)/i.test(text);
}

export function resolveStorefrontConstraintZh(context: unknown): string {
  return usesOpenFrontMallConstraint(context) ? STOREFRONT_CONSTRAINT_ZH : GENERIC_STOREFRONT_CONSTRAINT_ZH;
}

export function resolveStorefrontOpeningZh(context: unknown): string {
  return usesOpenFrontMallConstraint(context) ? STOREFRONT_OPENING_ZH : GENERIC_STOREFRONT_OPENING_ZH;
}

export function resolveStorefrontOpeningEn(context: unknown): string {
  return usesOpenFrontMallConstraint(context) ? STOREFRONT_OPENING_EN : GENERIC_STOREFRONT_OPENING_EN;
}

// 软清洗:把脚本里漏网的"推门 / 街边 / 马路"类描述替换成合理表述,作为兜底。
export function sanitizeStorefrontText(s: string, strictOpenFront = true): string {
  if (!s) return s;
  if (!strictOpenFront) return s;
  return s
