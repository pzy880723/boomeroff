...
import { assertNotSuspended, assertQuizSourceReadable, isValidKind } from "../_shared/quiz-access.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!auth.toLowerCase().startsWith("bearer ")) return json({ error: "未登录" }, 401);
    ...
    const { id, kind = "official", force = false } = body ?? {};
    if (!id || typeof id !== "string") return json({ error: "缺少 id" }, 400);
    if (!isValidKind(kind)) return json({ error: "kind 不合法" }, 400);
    ...
    // 题源一律通过带 JWT 的客户端读取（受 RLS 约束），未授权直接 403，不读取内容、不调用模型
    if (kind === "official") {
      const { data: row } = await userClient
        .from("official_knowledge").select("*").eq("id", id).maybeSingle();
      try { assertQuizSourceReadable("official", row, user.id); }
      catch (e: any) { return json({ error: e.message }, e.status ?? 403); }
      officialRow = row;
      ...
      const { data: fav } = await userClient
        .from("user_favorites").select("user_id, snapshot, source_type, source_id")
        .eq("id", id).maybeSingle();
      try { assertQuizSourceReadable("favorite", fav, user.id); }
      catch (e: any) { return json({ error: e.message }, e.status ?? 403); }
      ...
      const { data: k } = await userClient
        .from("product_knowledge")
        .select("product_name, category, era, origin, selling_points, tips")
        .eq("id", id).maybeSingle();
      try { assertQuizSourceReadable("knowledge", k, user.id); }
      catch (e: any) { return json({ error: e.message }, e.status ?? 403); }
      knowledge = {
