...
【写作要求】
- one_liner ≤30 字中文，全部正向表达；禁止「便宜/廉价/劣质/二手感/过时/淘汰/小众冷门/不值/平替」等负面或贬低词。可在身份定位（「日瓷里的礼器担当」）、时代符号（「装得下整个 80 年代的随身音乐」）、工艺亮点（「24K 描金，光下会呼吸」）、场景画面（「端上桌就是会客厅的 C 位」）、收藏价值（「停产即绝版的时代孤本」）、适度类比（「日瓷界的高定礼服」）等结构中灵活挑选；同一批词条至多出现一次「XX 的爱马仕/劳斯莱斯/iPhone」式类比。
