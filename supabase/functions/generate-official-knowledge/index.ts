        ...
        draft: {
          ...
          properties: {
            ...
            one_liner: {
              type: "string",
              description:
                "★最重要★ ≤30字中文金句，全部正向表达，禁止『便宜/廉价/劣质/二手感/过时/淘汰/小众冷门/不值/平替』等负面或贬低词；可用身份定位/时代符号/工艺亮点/场景画面/收藏价值/适度类比等多种结构，避免反复套用「XX 的劳斯莱斯/爱马仕」。",
            },
            quick_facts: {
              type: "array",
...
【写作要求】
- one_liner 必须是 ≤30 字的中文金句，让顾客/店员 3 秒记住，全部正向表达，禁止出现「便宜/廉价/劣质/二手感/过时/淘汰/小众冷门/不值/平替」等负面或贬低词汇。
- 金句可在以下结构中灵活挑选，避免长期套用同一种「XX 界的劳斯莱斯/爱马仕」公式（同一批生成里至多出现一次「XX 的爱马仕/劳斯莱斯/iPhone」式类比）：
  · 身份定位式：「日瓷里的礼器担当」「昭和家电的国民记忆」
  · 时代符号式：「装得下整个 80 年代的随身音乐」「平成办公桌上的小确幸」
  · 工艺亮点式：「一窑一画，金彩描得见笔触」「24K 描金，光下会呼吸」
  · 场景画面式：「端上桌就是会客厅的 C 位」「放进玄关，整面墙都安静下来」
  · 收藏价值式：「越用越润的传家器」「停产即绝版的时代孤本」
  · 适度类比式（节制使用）：「日瓷界的高定礼服」「昭和音响里的米其林」
- 类比对象优先选用「高定/礼器/传家宝/经典/殿堂/标杆/教科书/国民记忆/时代符号」等正向意象，避免奢侈品牌名连续重复。
...
【封面 cover_prompt 写作铁律】
- 只描述外观，不描述身份。绝对禁止出现任何品牌名、IP 名、角色名、设计师名、产品系列名、公司名、动漫/游戏/电影名（中英日罗马字、缩写一概禁止）。
- 不要出现专有名词或行业术语；用小学生能听懂的英文形容词去写：颜色、材质、形状、纹样、年代感（如 vintage / retro / modern）。
- 必须以 "on plain white background, soft natural light, centered, photorealistic, no text, no watermark, no logo" 收尾。
- 反例：『Koransha porcelain cup with red Akae pattern』『Sonny Angel baby figurine』『vintage Sony Walkman cassette player』
- 正例：『A small white porcelain coffee cup with hand-painted red flowers and gold rim, on plain white background, soft natural light, centered, photorealistic, no text』『A small vinyl baby figurine with glossy finish wearing a fruit-shaped hat, on plain white background, soft natural light, centered, photorealistic, no text』『A vintage portable cassette player in silver metal with black buttons and a small screen, on plain white background, soft natural light, centered, photorealistic, no text』

【行为】
- 若用户在追问中要求修改，请基于已存在的草稿做增量更新，未提及字段保持不变。
- 默认只返回用户本轮明确要修改的字段，不要把整张草稿原样回吐，更不要返回 body 长正文（除非用户明确说要改正文）。
- cover_prompt 默认不返回，仅当用户要求重画封面、当前没有封面、或外观相关字段（名称/品类/颜色/材质等）发生变化时才返回。
