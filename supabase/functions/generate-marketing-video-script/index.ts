// 生成「文生视频」脚本（结构化 JSON）。
// 主题 + 可选参考图 → 6 条镜头描述（每条含 video_prompt + 字幕 + 时长 + 可选参考图 index）。
...
- 参考图(image_index)是**可选**的：如果店员上传了相关参考图，就用它来约束画面风格/商品/场景；没合适的就填 null，让 video_prompt 完全自由发挥。
...
- 画幅 ${aspect}。
- 字幕一律简体中文，每行不超过 14 字。
- video_prompt 一律英文，每段不超过 60 个英文单词，必须包含主体/动作/镜头运动/光线四要素。
- 镜头条数 4–6 条；每条 2–5 秒。
- 不写"主播""直播间""保真""保证升值"等违禁词。`;
    ...
    const userPrompt = `视频主题：${topic || rule.label}
