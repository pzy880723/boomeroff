-- Keep BOOMER's background script flow idempotent across repeated taps and multiple devices.
-- Existing duplicates are retired before the partial unique index is created.
with ranked as (
  select
    id,
    row_number() over (
