...
test('DeepSeek 脚本规范化后五段对白、字幕和连续全文严格一致', () => {
  ...
  const script = normalizeDeepSeekSurpriseScript({
    ...
    hook: { scene: '真实门头', action: '边走边说', dialogue: dialogues[0], subtitle: dialogues[0], image_index: 0 },
    scenes: [
      { scene: '货架', action: '边走边说', dialogue: dialogues[1], subtitle: dialogues[1], image_index: 1 },
      { scene: '商品特写', action: '边拿边说', dialogue: dialogues[2], subtitle: dialogues[2], image_index: 2 },
      { scene: '翻筐体验', action: '边翻边说', dialogue: dialogues[3], subtitle: dialogues[3], image_index: 3 },
    ],
    outro: { scene: '店内全景', action: '边招手边说', dialogue: dialogues[4], subtitle: dialogues[4], image_index: 4 },
  });
